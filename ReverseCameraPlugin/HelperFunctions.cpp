...
std::string sp::vector_to_string(Vector v, int prec) {
        return to_string_decimals(v.X, prec) + ", " +
               to_string_decimals(v.Y, prec) + ", " +
               to_string_decimals(v.Z, prec);
}

std::string sp::rot_to_string(Rotator v, int prec) {
        return to_string_decimals(v.Pitch, prec) + ", " +
               to_string_decimals(v.Yaw, prec) + ", " +
               to_string_decimals(v.Roll, prec);
}

std::string sp::quat_to_string(Quat v, int prec) {
        return to_string_decimals(v.W, prec) + ", " +
               to_string_decimals(v.X, prec) + ", " +
               to_string_decimals(v.Y, prec) + ", " +
               to_string_decimals(v.Z, prec);
}
...
bool sp::rot_eq(Rotator r1, Rotator r2) {
        return (r1.Pitch == r2.Pitch) && (r1.Yaw == r2.Yaw) &&
               (r1.Roll == r2.Roll);
}

Vector sp::quatToFwd(Quat quat) {
        // Vector fwd = Vector(1 - 2 * (quat.X*quat.X + quat.Y*quat.Y), 2 *
        // (quat.Y*quat.Z + quat.X*quat.W), -1 * (2 * (quat.X*quat.Z -
        // quat.Y*quat.W)));
...
Vector sp::quatToRight(Quat quat) {
        // Vector right = Vector(2 * (quat.Y*quat.Z - quat.X*quat.W), 1 - 2 *
        // (quat.X*quat.X + quat.Z*quat.Z), -1 * (2 * (quat.X*quat.Y +
        // quat.Z*quat.W)));
...
Vector sp::quatToUp(Quat quat) {
        // Vector up = Vector(2 * (quat.X*quat.Z + quat.Y*quat.W), 2 *
        // (quat.X*quat.Y - quat.Z*quat.W), -1 * (1 - 2 * (quat.Y*quat.Y +
        // quat.Z*quat.Z)));
...
Rotator sp::quatToRot(Quat q) {
        ...
        hor_right = {-hor_right.X, -hor_right.Y,
                     -hor_right.Z};  // left-handed coordinate system
        hor_right.normalize();
