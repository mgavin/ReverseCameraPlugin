...
extern void LOG(const S & format_str, Args &&... arg);
...
private:
        ...
        struct HookedEventHash {
                std::size_t operator()(const std::shared_ptr<HookedEvent> &
                                               hooked_event) const {
                        return std::hash<std::string>{}(
                                hooked_event->eventName +
                                std::to_string(hooked_event->isPost));
                }
        };

        static std::unordered_set<std::shared_ptr<HookedEvent>,
                                  HookedEventHash,
                                  hook_cmp>
                hooked_events;
...
void HookedEvents::AddHookedEvent(
        ...
        if (std::find_if(begin(hooked_events), end(hooked_events),
                ...
                LOG("Hooked event \"{}\" already exists. Cannot overwrite.",
                    eventName);
                return;
        }
